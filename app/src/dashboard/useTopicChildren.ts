 ...
 */
export function useFleetAnomalySeverities(devices: ChildTopic[]): Severity[] {
 ...
 */
export function useDeviceSeverities(devices: ChildTopic[]): Record<string, Severity> {
  ...
  useEffect(() => {
    function refresh() {
      ...
      devices.forEach(d => {
        next[d.key] = deviceSeverity(d.node)
      })
      setSeverities(next)
    }
